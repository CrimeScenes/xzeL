...
local AllBodyParts = {
    ...
    "LeftLowerLeg", "RightLowerLeg", "RightUpperLeg", "RightFoot"
}

local boxSync = getgenv().TriggerBot.Settings.BoxSync or false  -- Retrieve BoxSync setting

local lastPosition = nil  -- For movement pattern recognition
local velocityEMA = Vector3.new(0, 0, 0)  -- For velocity smoothing (EMA)
local positionEMA = Vector3.new(0, 0, 0)  -- For position smoothing (EMA)

local function applyEMA(currentValue, previousEMA, smoothingFactor)
    return previousEMA * (1 - smoothingFactor) + currentValue * smoothingFactor
end
...
local function adjustAimForTarget(targetPosition)
    ...
    if onScreen then
        ...
        -- Apply smoothing
        local smoothingFactor = 0.1  -- Example smoothing factor, can be adjusted
        local smoothedOffsetX = applyEMA(offsetX, velocityEMA.X, smoothingFactor)
        local smoothedOffsetY = applyEMA(offsetY, velocityEMA.Y, smoothingFactor)

        VirtualInputManager:SendMouseMoveEvent(centerX + smoothedOffsetX, centerY + smoothedOffsetY, game)
    end
end
...
local function recognizeMovementPattern(character)
    if lastPosition then
        local velocity = (character.HumanoidRootPart.Position - lastPosition) / RunService.Heartbeat:Wait()
        velocityEMA = applyEMA(velocity, velocityEMA, 0.2)  -- EMA for velocity smoothing
    end
    ...
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then -- Skip knocked players
            ...
            if predictedPosition and isWithinBoxFOV(predictedPosition) then
                ...
                if closestPart then
                    if os.clock() - lastClickTime >= getgenv().TriggerBot.Settings.Cooldown then
...
RunService.RenderStepped:Connect(function()
    if isToggled then
        if boxSync then
            -- Box synchronization logic to keep the box aligned with the target
            if TargetPlayer and TargetPlayer.Character then
                ...
                if targetPart then
                    ...
                    -- Create or update the box GUI element
                    local boxGui = LocalPlayer.PlayerGui:FindFirstChild("BoxGui")
                    if not boxGui then
                        boxGui = Instance.new("ScreenGui")
                        boxGui.Name = "BoxGui"
                        boxGui.Parent = LocalPlayer.PlayerGui
                    end

                    local boxFrame = boxGui:FindFirstChild("TargetBox")
                    if not boxFrame then
                        boxFrame = Instance.new("Frame")
                        boxFrame.Name = "TargetBox"
                        boxFrame.Size = UDim2.new(0, boxSize.X, 0, boxSize.Y)
                        boxFrame.BorderSizePixel = 1
                        boxFrame.BorderColor3 = Color3.fromRGB(255, 0, 0)  -- Red border for visibility
                        boxFrame.BackgroundTransparency = 0.5
                        boxFrame.Parent = boxGui
                    end

                    -- Update box position
                    boxFrame.Position = UDim2.new(0, boxPosition.X, 0, boxPosition.Y)
                end
            end
        end
